...
class Csv(object):
    ...
    def validate_vlan_and_ip(self, entry, field, line_count):
        ...
        for it in items:
            item_count += 1
            if self.validate_vlan(it, item_count, line_count) is False:
                if self.validate_ip_range(it, item_count, line_count) is False:
        ...
        for item in items:
            ...
            if item.find("/") > 0:
                ...
                if self.is_number_in_range(mask, 1, 32) is False:
                    str = "ref: {}\nline #:{} - invalid mask {} : {}\n{}".format(comment, line_count, field_name, item, field)
                    l.message_user(str)
                    success = False
                    continue
            elif item.find(" mask ") > 0:
                aux = item.split(" mask ")
                four_octets = aux[0]
                mask = aux[1]
